;( function( $ ) {
    ...
    function wrapContainer ( container ) {
        ...
        if ( container.nodeType === 9 ) {
            ...
            $container = wrapContainer( $container[0].defaultView || $container[0].parentWindow );
            ...
            if ( ! ( container.contentWindow && container.contentWindow.document && container.contentWindow.document.childNodes && container.contentWindow.document.childNodes.length ) ) throw new Error( 'Invalid container: IFrame with inaccessible content' );
    ...
    function _prepareConfig ( $elem, container, opts ) {
        ...
        config.anyPart = opts.anyPart;
        config.zeroSize = opts.zeroSize;
     ...
     * @param {boolean}            config.anyPart            todo rename?
     * @param {boolean}            config.zeroSize           todo rename?
     *
     * @returns {boolean}
     */
    function _isInView ( elem, config ) {
        ...
        // If non-zero size is required, we check if an element consumes space in the document. And we out immediately
        // if the element doesn't meet the requirement.
        //
        ...
        if ( !( config.zeroSize || ( elem.offsetWidth > 0 && elem.offsetHeight > 0 ) ) ) return false;
